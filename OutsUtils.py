    ...
    def calculateOdds(self, cardsToCome, numUnknowns, outsLists):
        ...
        if cardsToCome == 1:
            ...
            for i in range(len(currentOutsLists)):
                ...
                oddsSoFar = (
                                Decimal(currentOutsLists[i].numCards) /
                                Decimal(numUnknowns)
                            )
                oddsSum += (oddsSoFar * (
                                self.calculateOdds(
...
def main():
    ...
    print('flop one pair NLH: ' +
          str(path.calculateOdds(3, 50, outsLists)))
    ...
    outsLists = path.outsLists
    print('flop two pair NLH: ' + str(path.calculateOdds(3, 50, outsLists)))

    outsList = OutsList(6, 1)
    missesList = OutsList(44, 4)
    path = Path([outsList, missesList])
    outsLists = path.outsLists
    print('one pair by the river NLH: ' +
          str(path.calculateOdds(5, 50, outsLists)))
    ...
    outsLists = path.outsLists
    print('flop two pair NLH: ' +
          str(path.calculateOdds(3, 50, outsLists) * 2))
